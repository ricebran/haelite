...
- **SC-004**: The system supports at least 60 paper-trading sessions before any
 - **SC-004**: The system supports at least 60 completed `PaperSession` records
   with `reconciliation_status = reconciled` before any live-capital proposal may
   be created or submitted. Failed, cancelled, partial, unreconciled, or
   disputed sessions do NOT count toward this threshold.
...
- `ValidationRun` (aka Trial): A deterministic execution of a backtest or
  validation fold tied to a `DataSnapshot`, code commit, and parameters.
- `PaperCandidate` (aka Candidate): A frozen strategy candidate derived from a
  completed `ValidationRun` and associated risk configuration and approvals.
- `PaperSession`: A recorded paper-trading session (one market day of paper
  trading activity) used to satisfy live-proposal eligibility counts.

Canonical entity names (use these exact terms when referring to persisted domain entities):

- `ValidationRun` (canonical) — use `Trial` only as an explanatory alias in the glossary.
- `PaperCandidate`
- `PaperSession`
- `PaperOrder`
- `PaperFill`
...
Use the exact canonical names above when referring to persisted entities, APIs, or tasks. Lowercase words such as "session" or "run" may be used for narrative text only when they do not refer to the formal entities above.
 - `PaperOrder`: A persisted paper order generated by the order-generation
   service; includes candidate_id, session_id, snapshot_id, timestamp,
   quantity, price, side, status, and broker_order_id.
 - `PaperFill`: A persisted fill/execution record linked to a `PaperOrder` and
   `PaperSession`, including fill price, quantity, timestamp, broker_fill_id,
   and settlement metadata.
 - `LiveProposal`: A submission representing a request to consider live-capital
   for a `PaperCandidate` (requires evidence and eligibility gates).
 - `ApprovalRecord`: Immutable record of an approval decision with actor_id,
   role, object_type, object_id, decision, rationale, and timestamp.
